...
type SettingStorageManager struct {
	mu           sync.RWMutex
	userManager  *collection.Manager[*common_models.User]
	usersStorage map[int]*UserStorage // Maps user IDs to their UserStorage
	UserManager  *collection.Manager[*common_models.User]
	iconLoader   *image_loader.ImageLoader
	ctx          context.Context
}

func NewSettingStorageManager() (*SettingStorageManager, error) {

	// Handler the storageManager
	storageManager := &SettingStorageManager{
		usersStorage: make(map[int]*UserStorage),
		ctx:          context.Background(),
	}

	var err error
	storageManager.UserManager, err = collection.NewCollectionManager[*common_models.User](config.GetPath("/data/users.json"))
	if err != nil {
		panic(err)
	}
...
func (us *SettingStorageManager) loadAllIcons() {
...
func (us *SettingStorageManager) GetAssetManager(c *gin.Context, userID int) (*collection.Manager[*model.Person], error) {
...
func (us *SettingStorageManager) periodicMaintenance() {
...
func (us *SettingStorageManager) RepositoryGetOriginalImage(userID int, filename string) ([]byte, error) {
	return us.usersStorage[userID].originalImageLoader.LoadImage(us.ctx, filename)
}

func (us *SettingStorageManager) RepositoryGetTinyImage(userID int, filename string) ([]byte, error) {
...
func (us *SettingStorageManager) RepositoryGetIcon(filename string) ([]byte, error) {
	return us.iconLoader.LoadImage(us.ctx, filename)
}

func (us *SettingStorageManager) GetUserStorage(c *gin.Context, userID int) (*UserStorage, error) {
...
func (us *SettingStorageManager) RemoveStorageForUser(userID int) {
	us.mu.Lock()
	defer us.mu.Unlock()
